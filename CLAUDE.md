...
| 目标 | 实际 vx | 实际 vy | 效率 | 参数 |
|------|---------|---------|------|------|
| vx=0.3, vy=0 | 0.207 | -0.000 | 69% | T=0.25, L=0.22 |
| vx=0, vy=0.2 | 0.052 | 0.087 | 44% | T=0.25, L=0.22 |
| vx=0.3, vy=0.2 | 0.207 | 0.068 | vx=69%, vy=34% | T=0.25, L=0.22 |
...
- **MPC Q 权重 (Q_vx, Q_vy) 影响很小**：瓶颈在接触物理，不在优化
- **stance 阻抗 Kp/Kd 对速度无影响**：MPC 前馈力主导，阻抗仅维持接触

**偏航（vyaw）局限性**：
- SRB MPC 力差动偏航效率 <2%，受限于 trot 对角支撑腿力矩相消
- `--target-vyaw` 参数可用，但实际效果极微弱
...
|------|--------|-----------|---------------|------|
| `--gait-type` | trot | trot | trot | trot / walk / pace / bound |
